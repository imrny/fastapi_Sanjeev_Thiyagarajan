...
def get_posts(db: Session = Depends(get_db), response_model=schemas.Post, get_current_user: int = Depends(oauth2.get_current_user)):
...
def update_post(id: int, updated_post: schemas.PostUpdate,  db: Session = Depends(get_db), get_current_user: int = Depends(oauth2.get_current_user)):
    ...
    if post == None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                            detail=f"post with id:{id} does not exist")
    #put_query.update({'title': 'updated title', 'content': 'updated content'}, synchronize_session=False)
